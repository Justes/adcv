...
def vit_base_patch16_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    model = VisionTransformer(img_size=224, patch_size=16, embed_dim=768, depth=12,
                              num_heads=12, representation_size=768 if has_logits else None,
                              num_classes=num_classes)
    return model


if __name__ == "__main__":
    pretrained_model = '../../../pretrained/vit_base_patch16_224_in21k-e5005f0a.pth'
    model = vit_base_patch16_224_in21k()
    state = torch.load(pretrained_model)
    # state1 = {k.replace('attn', 'atten'): v for k, v in state.items()}
